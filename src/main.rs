...
fn main() {
    ...
    let font_regular = Vec::from(include_bytes!("../asset/regular.ttf") as &[u8]);
    let font_regular: Font<'static> = Font::try_from_vec(font_regular).unwrap();
    let font_bold = Vec::from(include_bytes!("../asset/bold.ttf") as &[u8]);
    ...
    let (draw_text_inputs, text_height) = generate_draw_text_details(
        parsed_text, 
